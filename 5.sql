...
CREATE OR REPLACE FUNCTION product (int, int) RETURNS int AS $$
    SELECT $1 * $2
$$ LANGUAGE SQL IMMUTABLE STRICT;

CREATE AGGREGATE agg_product (
BASETYPE = int
,SFUNC = product
,STYPE = int
,INITCOND = 1
...
CREATE OR REPLACE FUNCTION evenly_divisible(int) RETURNS int AS $$
    WITH RECURSIVE r(idx, prime, count) AS (
        SELECT
            1
            ,1
            ,1
